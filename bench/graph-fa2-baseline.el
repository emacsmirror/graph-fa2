...
LEN: The active node count."
  (when-let* ((nodes (graph-fa2-ctx-nodes ctx))
    ...
    (let ((gc-cons-threshold most-positive-fixnum))
      (with-current-buffer (or (graph-fa2-ctx-bg-buffer ctx) (current-buffer))
        (dolist (edge edges)
          (let ((u-idx (if (vectorp edge) (aref edge 0) (car edge)))
                (v-idx (if (vectorp edge) (aref edge 1) (cdr edge))))
            (when (and (< u-idx len)
                       (< v-idx len))
              (let* ((u-coords (graph-fa2-normalise-coords (aref pos-x u-idx) (aref pos-y u-idx) (aref pos-z u-idx)))
